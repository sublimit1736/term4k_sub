...
TEST_CASE("ThemeStore lists merged user/system theme ids", "[services][ThemeStore]") {
    ...
    const auto ids = ThemeStore::listThemeIds();
    REQUIRE(ids == std::vector<std::string>{"tokyo-night", "tomorrow-night"});

    ThemeStore::clearThemeDirOverridesForTesting();
}

TEST_CASE("ThemeStore prefers user theme over system theme", "[services][ThemeStore]") {
    ...
    JsonUtils loaded;
    REQUIRE(ThemeStore::loadThemeById("paper", loaded));
    REQUIRE(loaded.get("text.primary") == "#111111");

    ThemeStore::clearThemeDirOverridesForTesting();
}

TEST_CASE("ThemeStore imports and exports user theme files", "[services][ThemeStore]") {
    ...
    REQUIRE(ThemeStore::importThemeFileToUserDir(sourcePath, &importedThemeId));
    ...
    REQUIRE(ThemeStore::exportThemeToUserDir("Flat Remix Light", exported));
