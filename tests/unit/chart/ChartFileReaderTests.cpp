...
"[services][ChartIO]"
 ...
 {
    REQUIRE(ChartFileReader::num2hex8str(0x1Au) == "0000001a");
    REQUIRE(ChartFileReader::num2hex2str(0xAFu) == "af");
}
...
"[services][ChartIO]"
)
 {
    ...
    ChartBuffer buffer;
    REQUIRE(ChartFileReader::readChart(chartPath.string().c_str(), buffer, 2, 120.0f, {4, 4}));
...
"[services][ChartIO]"
)
 {
    ...
    REQUIRE_FALSE(ChartFileReader::readChart(badPath.string().c_str(), badBuffer, 1, 120.0f, {4, 4}));

    RuntimeConfig::resetToDefaults();
    RuntimeConfig::chartOffsetMs = -2500;
    RuntimeConfig::chartPreloadMs = 2000;

    const auto clampPath = temp.path() / "clamp.t4k";
    writeTextFile(clampPath, "t4kcb\n010000000064\nt4kce\n");

    ChartBuffer clampBuffer;
    REQUIRE(ChartFileReader::readChart(clampPath.string().c_str(), clampBuffer, 1, 120.0f, {4, 4}));
    REQUIRE(clampBuffer.find(0) != clampBuffer.end());

    RuntimeConfig::resetToDefaults();
    RuntimeConfig::chartOffsetMs = 0;
    RuntimeConfig::chartPreloadMs = 2600;

    const auto preloadPath = temp.path() / "preload.t4k";
    writeTextFile(preloadPath, "t4kcb\n010000000064\nt4kce\n");

    ChartBuffer preloadBuffer;
    REQUIRE(ChartFileReader::readChart(preloadPath.string().c_str(), preloadBuffer, 1, 120.0f, {4, 4}));
...
"[services][ChartIO]"
)
 {
    ...
    REQUIRE(ChartFileReader::readChart(absoluteChart.string().c_str(), absoluteBuffer, 1, 120.0f, {4, 4}));
    ...
    REQUIRE_FALSE(ChartFileReader::readChart(missingPath.string().c_str(), missingBuffer, 1, 120.0f, {4, 4}));
}
...
"[services][ChartIO]"
)
 {
    ...
    RuntimeConfig::resetToDefaults();
    RuntimeConfig::chartOffsetMs = 0;
    RuntimeConfig::keyBindings = {static_cast<uint8_t>('x')};
    ...
    ChartBuffer buffer;
    REQUIRE(ChartFileReader::readChart((temp.path() / "mixed.t4k").string().c_str(), buffer, 1, 120.0f, {4, 4}));
...
"[services][ChartIO]"
)
 {
    ...
    REQUIRE_FALSE(ChartFileReader::readChart(nullptr, nullBuffer, 1, 120.0f, {4, 4}));
    REQUIRE(nullBuffer.empty());

    ChartBuffer emptyBuffer;
    REQUIRE_FALSE(ChartFileReader::readChart("", emptyBuffer, 1, 120.0f, {4, 4}));
