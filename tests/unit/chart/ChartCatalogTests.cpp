...
"ChartCatalog loads chart metadata and merges best stats"
,
"[services][ChartCatalog]"
)
 {
    ...
    REQUIRE(RecordStore::addRecord("7 chart_alpha song userA 900000 97.50 1000 111"));
    REQUIRE(RecordStore::addRecord("7 chart_alpha song userA 880000 98.20 1200 222"));
    REQUIRE(RecordStore::addRecord("8 chart_alpha song userB 999999 99.99 1300 333"));

    const auto items = ChartCatalog::loadCatalogForUID(chartsRoot.string(), "7");
...
"ChartCatalog ignores folders that violate fixed file/id rules"
,
"[services][ChartCatalog]"
)
 {
    ...
    const auto items = ChartCatalog::loadCatalogForUID(chartsRoot.string(), "", &failures);
...
"ChartCatalog supports all requested sort keys and orders"
,
"[services][ChartCatalog]"
)
 {
    ...
    auto sorted = ChartCatalog::sortCatalogKeys(items, ChartListSortKey::DisplayName, SortOrder::Ascending);
    ...
    sorted = ChartCatalog::sortCatalogKeys(items, ChartListSortKey::Difficulty, SortOrder::Descending);
    ...
    sorted = ChartCatalog::sortCatalogKeys(items, ChartListSortKey::BestAccuracy, SortOrder::Ascending);
...
"ChartCatalog compliance check reports playable-note conflicts"
,
"[services][ChartCatalog]"
)
 {
    ...
    const auto conflicts = ChartCatalog::checkChartCompliance(chartPath.string(), 1);
