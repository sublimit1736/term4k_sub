...
bool RuntimeConfigs::loadForUser(const std::string &username) {
    ...
    hitSoundVolume = parseFloat(json.get("audio.hitSoundVolume", floatToString(hitSoundVolume)), hitSoundVolume, 0.0f, 1.0f);
    audioBufferSize = parseUint32(json.get("audio.bufferSize", std::to_string(audioBufferSize)), audioBufferSize, 64, 8192);

    showEarlyLate = parseBool(json.get("gameplay.showEarlyLate", showEarlyLate ? "true" : "false"), showEarlyLate);
    showAPIndicator = parseBool(json.get("gameplay.showAPIndicator", showAPIndicator ? "true" : "false"), showAPIndicator);
    showFCIndicator = parseBool(json.get("gameplay.showFCIndicator", showFCIndicator ? "true" : "false"), showFCIndicator);
    chartOffsetMs = parseInt32(json.get("gameplay.chartOffsetMs", std::to_string(chartOffsetMs)), chartOffsetMs);
    chartPreloadMs = parseUint32(json.get("gameplay.chartPreloadMs", std::to_string(chartPreloadMs)), chartPreloadMs, 0, 60000);
