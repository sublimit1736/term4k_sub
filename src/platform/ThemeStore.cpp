...
std::string ThemeStore::userDirOverride;
std::string ThemeStore::systemDirOverride;

std::string ThemeStore::userThemesDir() {
...
std::string ThemeStore::systemThemesDir() {
...
std::string ThemeStore::normalizeThemeId(const std::string &themeId) {
...
std::string ThemeStore::makeThemePath(const std::string &dir, const std::string &themeId) {
    if (dir.empty() || themeId.empty()) return "";
    return ensureTrailingSlash(dir) + themeId + ".json";
}

bool ThemeStore::ensureDir(const std::string &dir) {
...
std::vector<std::string> ThemeStore::listThemeIds() {
...
bool ThemeStore::loadThemeById(const std::string &themeId, JsonUtils &out) {
...
bool ThemeStore::themeExists(const std::string &themeId) {
    JsonUtils loaded;
    return loadThemeById(themeId, loaded);
}

bool ThemeStore::exportThemeToUserDir(const std::string &themeId, const JsonUtils &theme) {
    const std::string normalizedId = normalizeThemeId(themeId);
    if (normalizedId.empty()) return false;
...
bool ThemeStore::importThemeFileToUserDir(const std::string &sourceFilePath, std::string *outThemeId) {
...
void ThemeStore::setThemeDirOverridesForTesting(const std::string &userDir, const std::string &systemDir) {
    userDirOverride = userDir;
    systemDirOverride = systemDir;
}

void ThemeStore::clearThemeDirOverridesForTesting() {
