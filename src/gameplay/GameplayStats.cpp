...
void GameplayStats::reset(const uint32_t noteCount, const uint64_t maxScore) {
    perfectCount_ = 0;
    greatCount_   = 0;
    missCount_    = 0;
    earlyCount_   = 0;
...
void GameplayStats::settleNote(const GameplayJudgement judgement, const bool isEarly, const bool isLate) {
...
uint32_t GameplayStats::chartNoteCount() const {
    return chartNoteCount_;
}

uint64_t GameplayStats::chartMaxScore() const {
...
GameplayFinalResult GameplayStats::buildFinalResult() const {
...
void GameplayStats::updateAccuracy() {
