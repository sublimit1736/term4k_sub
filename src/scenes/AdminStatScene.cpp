...
namespace {
    ...
    std::map<std::string, AdminPlayerStats> buildStats(const std::vector<std::string> &records,
        ...
        for (auto &[uid, parsedRecords]: parsedByUid){
            const auto catalogById = ChartCatalog::loadCatalogForUID(chartsRoot, uid);
...
bool AdminStatScene::refresh(const std::string &chartsRoot) {
    if (!UserContext::isAdminUser()){
        verifiedStats.clear();
        allStats.clear();
        return false;
    }

    verifiedStats = buildStats(UserContext::loadAllVerifiedRecords(), chartsRoot);
    allStats      = buildStats(UserContext::loadAllRecords(), chartsRoot);
    return true;
}

const std::map<std::string, AdminPlayerStats> &AdminStatScene::playerStats(const AdminRecordScope scope) const {
    return scope == AdminRecordScope::VerifiedOnly ? verifiedStats : allStats;
}

const ChartRecordCollection *AdminStatScene::recordsByUser(const AdminRecordScope scope,
...
const ChartRecordCollection *AdminStatScene::recordsByUserAndChart(const AdminRecordScope scope,
                                                                      const std::string &uid,
