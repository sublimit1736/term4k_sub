...
void AudioPlayer::data_callback(ma_device* pDevice, void* pOutput, const void* /*pInput*/, ma_uint32 frameCount) {
    auto* svc = static_cast<AudioPlayer*>(pDevice->pUserData);
...
bool AudioPlayer::loadSong(const char* filename) {
    if (initialized) stop();
    const auto result = ma_decoder_init_file(filename, nullptr, &decoder);
    if (result != MA_SUCCESS){
        ErrorNotifier::notify(I18n::instance().get("error.decoder_init_failed"));
        initialized = false;
        return false;
    }
...
bool AudioPlayer::playSong() {
    if (!initialized){
        ErrorNotifier::notify(I18n::instance().get("error.device_not_initialized"));
        return false;
    }

    if (ma_device_start(&device) != MA_SUCCESS){
        ErrorNotifier::notify(I18n::instance().get("error.device_start_failed"));
        return false;
    }
...
bool AudioPlayer::seekToMs(uint32_t ms) {
    if (!initialized){
        ErrorNotifier::notify("SongPlayer::seekToMs", I18n::instance().get("error.device_not_initialized"));
        return false;
    }
