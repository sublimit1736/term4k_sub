#include "GameplayRecordService.h"
...
bool GameplayRecordService::saveFinalResult(const GameplayFinalResult &result,
                                            const std::string &chartID,
                                            const std::string &songName,
                                            const uint32_t timestampSec
    ) {
    if (chartID.empty()) return false;
    if (!AuthenticatedUserService::hasLoggedInUser()) return false;
    if (AuthenticatedUserService::isAdminUser() || AuthenticatedUserService::isGuestUser()) return false;
    ...
    return ProofedRecordsDAO::addRecord(record.str());
}
