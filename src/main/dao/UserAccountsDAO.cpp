...
std::string UserAccountsDAO::accountList = "users.db";

// Sets account database path for tests and runtime redirection.
void UserAccountsDAO::setDataDir(const std::string &dir) {
    if (dir.empty() || dir == "."){
...
std::string UserAccountsDAO::getField(const std::string &line, size_t idx) {
...
namespace {
    bool parseUint32Strict(const std::string &text, uint32_t &value) {
        ...
        try{
            ...
            ErrorNotifier::notifyException("UserAccountsDAO::parseUint32Strict", ex);
            return false;
        }
        catch (...){
            ErrorNotifier::notifyUnknown("UserAccountsDAO::parseUint32Strict");
            return false;
        }
    }
}
...
bool UserAccountsDAO::addUser(const std::string &username, const std::string &password, uint32_t uid) {
    if (username.empty() || password.empty()){
        ErrorNotifier::notify("UserAccountsDAO::addUser", I18nService::instance().get("error.auth_empty_credentials"));
        return false;
    }
        ...
        while (std::getline(in, line)){
            if (getField(line, 0) == username){
                ErrorNotifier::notify("UserAccountsDAO::addUser",
                                      I18nService::instance().get("error.auth_username_exists") + ": " + username);
                return false;
            }
    ...
    if (!out){
        ErrorNotifier::notify("UserAccountsDAO::addUser",
                              I18nService::instance().get("error.auth_account_file_open_failed") + ": " + accountList);
        return false;
    }
...
std::vector<std::string> UserAccountsDAO::readAllUsernames() {
