...
ChartDetectionFailure makeFailure(const fs::path &folder,
    ...
    return {folder.string(), issue, std::move(metadataID), I18n::instance().get(issueI18nKey(issue))};
}
...
bool tryBuildChartItem(const fs::path &folder,
    ...
    if (!in.is_open()) {
        ErrorNotifier::notify("ChartCatalog::tryBuildChartItem",
                              I18n::instance().get("error.metadata_file_open_failed") + ": " +
                                  metadataPath.string());
        if (failure != nullptr) {
...
std::vector<PlayableNoteConflict> ChartCatalog::checkChartCompliance(const std::string &chartFilePath,
    ...
    if (!chartFile.is_open()) {
        ErrorNotifier::notify("ChartCatalog::checkChartCompliance",
                              I18n::instance().get("error.chart_open_failed") + ": " + chartFilePath);
        return conflicts;
    }
    ...
    if (beginMarker != "t4kcb") {
        ErrorNotifier::notify("ChartCatalog::checkChartCompliance",
                              I18n::instance().get("error.chart_invalid_format") + ": " + chartFilePath);
        return conflicts;
    }
    ...
    for (const auto &conflict : conflicts) {
        ErrorNotifier::notifyWarning(
            "ChartCatalog::checkChartCompliance",
            I18n::instance().get("warning.chart_compliance_conflict") +
...
ChartCatalogMap ChartCatalog::loadCatalogForUID(const std::string &chartsRoot,
                                                       const std::string &uid,
    ...
    if (!fs::exists(root) || !fs::is_directory(root)) {
        ErrorNotifier::notify("ChartCatalog::loadCatalogForUID",
                              I18n::instance().get("error.charts_root_not_directory") + ": " + chartsRoot);
        return items;
    }
...
std::vector<std::string> ChartCatalog::sortCatalogKeys(const ChartCatalogMap &items,
