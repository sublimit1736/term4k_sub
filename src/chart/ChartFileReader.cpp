...
namespace {
    uint32_t applyTimingConfig(uint32_t rawTime) {
        ...
        const int64_t shift = static_cast<int64_t>(RuntimeConfig::chartOffsetMs) +
                              static_cast<int64_t>(RuntimeConfig::chartPreloadMs) -
    ...
    bool getMappedKey(uint8_t track, uint8_t &mappedKey) {
        if (track >= RuntimeConfig::keyBindings.size()) return false;
        mappedKey = RuntimeConfig::keyBindings[track];
        return true;
    }
...
constexpr uint8_t ChartFileReader::hexVal(char c) noexcept {
...
bool ChartFileReader::readChart(const char* fileName, ChartBuffer &chartBuffer,
    ...
    if (!chartFile){
        ErrorNotifier::notify(
                              I18n::instance().get("error.chart_open_failed") + ": " +
                              (resolvedPath.empty() ? I18n::instance().get("error.empty_path") : resolvedPath));
        return false;
    }
    ...
    if (chartSignBegin == "t4kcb"){
        ...
        while (std::getline(chartFile, itemHandle)){
            ...
            if (itemType == tap){
                ...
                if (track >= keyCount){
                    ErrorNotifier::notify(
                                          I18n::instance().get("error.tap_track_out_of_range") +
                ...
                if (track >= keyCount){
                    ErrorNotifier::notify(
                                          I18n::instance().get("error.hold_track_out_of_range") +
                                          " (track=" + std::to_string(static_cast<int>(track)) +
                                          ", keyCount=" + std::to_string(keyCount) + ")");
                    continue;
                }
    ...
    ErrorNotifier::notify(I18n::instance().get("error.chart_invalid_format"));
    return false;
}

// Converts uint32 to an 8-char hex string.
std::string ChartFileReader::num2hex8str(uint32_t num) {
    std::string s(8, '0');
    for (int i = 7; i >= 0; --i){
        s[i] = chex[num & 0xF];
        num  >>= 4;
...
std::string ChartFileReader::num2hex2str(uint8_t num) {
    std::string s(2, '0');
    s[0] = chex[(num >> 4) & 0xF];
    s[1] = chex[num & 0xF];
    return s;
...
uint32_t ChartFileReader::parseHexField(const std::string &str, int start) {
    uint32_t val = 0;
...
uint8_t ChartFileReader::parseHexField2(const std::string &str, int start) {
    uint8_t val = 0;
